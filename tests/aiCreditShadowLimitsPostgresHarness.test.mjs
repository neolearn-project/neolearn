...
const database = "neolearn_ai_credit_stage3_fixture";
...
test("Stage 3 shadow limit reporting behaves on PostgreSQL 17", { timeout: 120000 }, async () => {
  assert.match(image, /^postgres:17(?:\D|$)/);
  const container = `neolearn-ai-credit-stage3-${process.pid}-${randomBytes(5).toString("hex")}`;
  try {
    ...
    const fixture = psql(container, ["-f", "/workspace/tests/postgres/aiCreditShadowLimits.behavior.sql"]);
    assert.equal(fixture.status, 0, fixture.stderr || fixture.stdout);

    const sql = `select public.read_ai_credit_usage_shadow('student:windows',
      (select id from public.ai_credit_entitlement_periods where entitlement_identity='windows:p1'),
      '2026-01-10T12:00:00Z')::text`;
    const reads = await Promise.all(Array.from({ length: 8 }, () => concurrentPsql(container, sql)));
    ...
    const counts = psql(container, ["-A", "-t", "-c",
      "select count(*) from public.ai_credit_transactions"]);
    assert.equal(counts.stdout.trim(), "18", "reads changed persisted state");
  } finally {
